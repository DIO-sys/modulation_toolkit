                   ...
                   bool use_multipath)
{
    ...
    // compute LCM of frame size and interleaver block size
    // so padding aligns to both simultaneously
    ...
    for (float snr_db : snr_points) {
        ...
        // pad to LCM of frame size and interleaver block
        size_t padded_len = ((interleaved.size() + lcm_size - 1)
                             / lcm_size) * lcm_size;
        interleaved.resize(padded_len, 0);

        // QPSK map — 2 bits per symbol, no RRC, no upsampling
        ...
        // QPSK soft demap — real/imag are natural LLRs
        ...
        // trim to interleaved size — no padding needed since
        // LCM alignment guarantees divisibility
        if (llrs.size() > padded_len) llrs.resize(padded_len);

        // deinterleave full blocks
